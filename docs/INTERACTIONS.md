...
- 用户可以点“跳过”，前端 90 秒也会自动继续；未填项按“你来判断 / 未指定”处理
- 收到 `用户对上一条 <question-form> 的回答：` 后，本轮任务不得再输出第二张表单；缺失项由设计专家自行判断
...
| `questions[].customLabel` | string | optional | 自定义输入前缀，默认 `其他` |
| `questions[].customPlaceholder` | string | optional | 自定义输入 placeholder，默认 `我来告诉你...` |
| `submitLabel` | string | optional | 提交按钮文案，≤ 24 字 |
...
- 每题选项建议：radio 3-5 项，checkbox 4-8 项，必须包含一个 `你来判断` / `不确定` 兜底项
- radio / checkbox / select / direction-cards 每个问题后面都会有一个固定选项之外的输入框，用于用户写「其他 / 我来告诉你」内容
- 优先使用高密度 chip 选择：短问题 + 短选项；避免大段方案文本塞进选择项
- 只有视觉方向确实需要色板/字体样张时才用 `direction-cards`，否则用 radio chips
- 每张 Phase 1 表单最后必须放一个可选文本题：`id: "notes"` / `label: "补充说明"` / `type: "textarea"`，让用户一次性补充未覆盖的信息

### Deck / PPT 类 Phase 1 表单硬规则

当 `TASK_ROUTER.md` 判定 `used_mode == "deck"`，或用户首句包含 PPT / slides / presentation / deck / 演示 / 幻灯片 / 分享稿时，Phase 1 **必须**先输出一张 deck 表单。即使用户给了 PDF、论文、页数、风格或“你来判断”，也不能直接开始做。表单一次包含 6-10 个问题，问题 ID 优先使用下列语义，避免后续二次确认：
...
- 前端会回传：
  - `用户对上一条 <question-form> 的回答：`
  - 后续每行形如 `- {label}: {displayValue}`
- 收到回复后，按 `label` 对应原问题消费答案
- `未指定` 表示用户跳过该项
- 收到表单回复后，不要重复输出同一张表单；应写回 `USER.md` 并继续下一步

### 前端约定（无需在 payload 中声明，模型仅作了解）

- **卡片标题**：前端写死为 `"信息补充"`；payload `title` 仅作为语义提示
- **跳过按钮**：前端默认显示「跳过」按钮，文案与行为均由前端控制
- **底部说明文案**：前端写死为 `"请补充信息，如选择跳过，设计专家将自行判断需求并执行任务"`
- **自动继续**：前端在底部显示 90 秒倒计时；用户无操作超时后会按当前已填内容自动提交，未填项按「未指定」处理；用户点选或输入后倒计时重新开始
- **自定义输入**：选择题后置的「其他 / 我来告诉你」输入由前端自动渲染，不需要把它写进 `options`
